"""
Bước 11 — Pairwise ML Classifier trên đặc trưng VGG19 (Transfer Learning + ML)

(Reference, Query) -> |VGG19_feature_ref - VGG19_feature_query| -> RF/SVM -> Match/Non-match

Cấu trúc giống hệt 08_pairwise_baseline.py (cùng writer_splits.json, cùng
quy trình feature selection chỉ trên train, threshold trên validation, test
1 lần duy nhất) — CHỈ khác nguồn đặc trưng: dùng vector 512 chiều từ VGG19
thay vì 11 đặc trưng thủ công (Hu Moments, GLCM...).
"""
import json
import numpy as np
import pandas as pd
...
RESULTS_DIR = Path("results/tables/vgg19_pairwise")
ALPHA = 0.01
TARGET_FAR = 0.10
...
def select_diff_features_train_only(X_train, y_train, feature_cols, alpha=ALPHA, max_features=MAX_FEATURES_SELECTED):
...
def main():
    ...
    features = pd.read_csv(PROC_DIR / "features_vgg19.csv")
    feature_cols = [c for c in features.columns if c.startswith("vgg_")]
    print(f"Số chiều đặc trưng VGG19: {len(feature_cols)}")
    ...
    print("\n" + "=" * 70)
    print(f"TỔNG HỢP QUA {len(splits)} SPLIT — VGG19 Transfer Learning + ML Classifiers")
    print("=" * 70)
